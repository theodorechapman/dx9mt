...
- `build/dx9mt_metal_viewer` (native): standalone Metal renderer reading double-buffered IPC shared memory.
...
- Multi-texture: all 8 sampler stages (0-7) transmitted and consumed per-draw.
- IPC: 16MB double-buffered shared memory (two ~8MB slots) with lock-free sequence protocol, per-draw entries, bulk VB/IB/texture/constant/shader data.
- Metal rendering: vertex declarations, indexed draws, WVP transform, texture sampling, alpha blending, cull mode.
...
- Fail-fast hardening: fatal abort on nil PSO/buffer/encoder, draws skipped on shader translation failure (no silent fallback to wrong path), loud logging on all stub/skip paths.
